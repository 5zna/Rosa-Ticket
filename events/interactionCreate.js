...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (interaction.isButton()) {
      ...
      const supportRoles = cat?.supportRoles?.length ? cat.supportRoles : (config.supportRoles || []);
      const managerRoles = cat?.managerRoles?.length ? cat.managerRoles : (config.managerRoles || []);
      ...
      if (interaction.customId === 'add') {
        if (!isStaff) return interaction.reply({ content: '❌ Only support staff can add users.', ephemeral: true });
        const modal = new ModalBuilder()
      ...
      if (interaction.customId === 'blacklist') {
        if (!isStaff) return interaction.reply({ content: '❌ Only support staff can blacklist.', ephemeral: true });
        const ticketInfo = ticketStore.getByThreadId(interaction.channel.id);
