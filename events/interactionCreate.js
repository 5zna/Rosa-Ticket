  ...
  async execute(interaction) {
    if (interaction.isStringSelectMenu() && interaction.customId === 'ticket_menu') {
      ...
      try {
        ...
        try { await interaction.editReply({ content: '❌ An error occurred while creating your ticket.' }); } catch {}
      }
      return;
    }
    ...
    if (interaction.isButton()) {
      ...
      const ticketInfo = await ticketStore.getByThreadId(interaction.channel.id);
      ...
      if (interaction.customId === 'claim') {
        ...
        await ticketStore.update(interaction.channel.id, { claimedBy: claimerId });
      ...
      if (interaction.customId === 'unclaim') {
        ...
        await interaction.deferUpdate();
        await ticketStore.update(interaction.channel.id, { claimedBy: null });
        const ownerId = ticketInfo?.userId;
      ...
      if (interaction.customId === 'blacklist') {
        ...
        await blacklistStore.add(userId);
        await ticketStore.remove(interaction.channel.id);
      ...
      if (interaction.customId === 'confirm_close') {
        ...
        if (ticketInfo) {
          ...
          await ticketStore.update(interaction.channel.id, { closed: true });
          const btns = config.buttons || {};
          const b = (key, style) =>
            new ButtonBuilder()
              .setCustomId(key)
      ...
      if (interaction.customId === 'reopen') {
        ...
        await ticketStore.update(interaction.channel.id, { closed: false });
        const btns = config.buttons || {};
        const b = (key, style) =>
          new ButtonBuilder()
            .setCustomId(key)
