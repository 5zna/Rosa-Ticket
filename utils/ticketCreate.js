...
module.exports = async (interaction, type, description) => {
  ...
  if (await blacklistStore.isBlacklisted(interaction.user.id)) {
  ...
  const catManagerRoles = config.managerRoles || [];
  const catMentionRoles = config.mentionRoles || [];
  ...
  const mentionMsg = await thread.send({ content: [mentionPings, `<@${interaction.user.id}> \n يرجى طرح مشكلتك لحين تواجد الدعم`].filter(Boolean).join(' '), components: [row] });
  ...
  const allOpts = rawOpts.map(o => ({
    ...o,
    label: menus[o.value] === false ? `${o.label} (Disabled)` : o.label,
    description: menus[o.value] === false ? 'This option is currently disabled' : o.description,
  }));
  ...
  try {
    const panelCh = interaction.guild.channels.cache.get(config.panelChannelId);
    if (panelCh && config.panelMessageId) {
      const panelMsg = await panelCh.messages.fetch(config.panelMessageId);
      await panelMsg.edit({ components: [freshMenu] });
    }
  } catch {}

  await ticketStore.add({
